...
if(!chang){
    str='<div class="slide-wrapper"><div class="header"><em class="cancel">取消</em> <em></em><em class="done">确定</en> </div>'+
    ...
    slideContainer.addEventListener("click",function(e){
		...
		var lists=document.querySelectorAll(".slide-wrapper span")
        var cont="";
        if(e.target.tagName == 'LI' || e.target.tagName == 'SPAN'){
            for(var i=0; i<list.length;i++){
                list[i].classList.remove('slide-selected');
				lists[i].classList.remove('slide-selected');
			}
